...
public class RegistroStudenti {
	...
	public void aggiungiStudente(String nome, String cognome, String matricola) {
		...
		// con un ciclo for each serve a scorrere la lista dell'oggetto studente e fa un
		// confronto della matricola
	...
	public void visualizzaStudenti() {
		if (studenti.isEmpty()) {
			// controlla se la lista studenti è vuota allora stampa un mexx che non ci sono
			// studenti nel registro
			System.out.println("Non ci sono studenti nel registro.");
		} else {// altrimenti si stampa prima elenco studenti
			System.out.println("\nElenco degli studenti:");
			for (Studente s : studenti) {
				System.out.println(s);
				// e poi si fa un for each per stampare tutto l elenco degli studenti presenti
				// nel registro
			}
		}
	}
	...
	public void cercaStudente(String matricola) {
		...
		for (Studente s : studenti) {
			// e poi controlla se la matricola inserita in ingresso corrisponde ad una
			// inserita in ingresso
			if (s.getMatricola().equals(matricola)) {
				// singolo studente.getmatricola (elemento univoco di ogni studenti).equals
				// matricola
